...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const REDIRECT_URI = process.env.GOOGLE_REDIRECT_URI || "http://localhost:3000/api/auth/youtube/callback";
    ...
    if (!tokens.access_token || !tokens.refresh_token) {
      console.error("[youtube/callback] Missing tokens:", JSON.stringify(tokens));
    ...
    try {
      ...
      console.warn("[youtube/callback] Channel fetch error:", e);
    }
    ...
    if (dbErr) {
      console.error("[youtube/callback] DB error:", dbErr);
      return NextResponse.redirect(new URL("/dashboard/settings?youtube=error&reason=db_error", req.url));
    }

    console.log("[youtube/callback] Connected YouTube:", channelTitle);
