...
export default function RepurposeHistoryPage() {
  ...
  async function publishToYouTube(clip: RepurposeClip, projectId: string) {
    if (!clip.video_url) return;
    const clipKey = clip.id || `${projectId}-${clip.index}`;
  ...
  /* ── Publish all clips in a project ──────────────────────── */
  async function publishAllClips(project: RepurposeProject) {
    const clips = (project.clips || []).filter((c) => c.video_url);
    if (clips.length === 0) return;

    for (const clip of clips) {
      const clipKey = clip.id || `${project.id}-${clip.index}`;
      if (publishing[clipKey] === "published") continue; // Skip already published
      await publishToYouTube(clip, project.id);
    }
  }
                  ...
                  {isExpanded && (
                    <div className="px-4 pb-4 border-t border-gray-800/50">
                      ...
                      <div className="flex items-center gap-2 py-3">
                        ...
                        {doneClips.length > 0 && youtubeConnected && (
                          <button
                            onClick={() => publishAllClips(project)}
                            className="px-3 py-1.5 rounded-lg text-xs font-medium text-white transition-all flex items-center gap-1.5 hover:scale-[1.02] active:scale-[0.98]"
                            style={{
                            ...
                            >
                                ...
                                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2">
                                  {clip.video_url && (
                                    <>
                                      ...
                                      {/* YouTube publish button in overlay */}
                                      {(() => {
                                        const clipKey = clip.id || `${project.id}-${clip.index}`;
                                        ...
                                        if (pubStatus === "published" && pubResult?.url) {
                                          return (
                                            <a
                                              ...
                                              className="px-2.5 py-1 rounded text-[10px] font-medium backdrop-blur-sm"
                                        ...
                                        if (pubStatus === "uploading") {
                                          return (
                                            <span className="px-2.5 py-1 rounded text-[10px] font-medium text-yellow-300 backdrop-blur-sm" style={{ background: "rgba(251,191,36,0.2)" }}>
                                        ...
                                        if (pubStatus === "error") {
                                          return (
                                            <button
                                              onClick={() => publishToYouTube(clip, project.id)}
                                              className="px-2.5 py-1 rounded text-[10px] font-medium backdrop-blur-sm"
                                        ...
                                        if (youtubeConnected) {
                                          return (
                                            <button
                                              onClick={() => publishToYouTube(clip, project.id)}
                                              className="px-2.5 py-1 rounded text-[10px] font-medium text-white backdrop-blur-sm hover:scale-105 transition"
                              ...
                              <div className="p-2">
                                ...
                                {(() => {
                                  ...
                                  if (pubStatus === "published" && pubResult?.url) {
                                    return (
                                      <a
                                        ...
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="flex items-center gap-1 mt-1 text-[9px] text-green-400 hover:text-green-300"
                                      >
                                        ✅ YouTube ↗
                                      </a>
                                    );
                                  ...
                                  if (pubStatus === "uploading") {
                                    return (
                                      <span className="flex items-center gap-1 mt-1 text-[9px] text-yellow-400">
                                  ...
                                  if (pubStatus === "error") {
                                    return (
                                      <span className="mt-1 text-[9px] text-red-400" title={pubResult?.error}>
                                        ⚠ Failed
                                      </span>
                                    );
                                  }
